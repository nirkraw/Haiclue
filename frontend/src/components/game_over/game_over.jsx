  ...
  render() {
    ...
    const scores = playersArray.map((player, idx) => {
      return (
        <li key={idx}>
          {player.handle}
          <span className="points color-blue"> {player.points}</span>{" "}
        </li>
      );
    });
    ...
    const winner =
      tie.length > 1 ? (
        <h1>
          {tie.slice(0, tie.length - 1).join(", ")} and {tie[tie.length - 1]}{" "}
          tie!
        </h1>
      ) : (
    ...
    return (
      <div className="game-over-container">
        ...
        <div className="game-over-buttons">
          <button onClick={this.playAgain} className="game-over-button">
            Play Again!
          </button>
          <button onClick={this.newRoom} className="game-over-button">
            New Room
          </button>
        </div>
      </div>
