...
class ClueContruction extends React.Component {
  ...
  render() {
    ...
    let clueConstruction = this.props.clueConstructionArray.map(
      (tile, index) => {
          ...
          return (
            <span key={index * 1000}>
              <div
                ...
                className={`color-${currentColor} tile add-line`}
              >
                &#8629;
              </div>
              <br />
    ...
    return (
      <div>
        <button onClick={this.submitClue} id="clue-submit-button">
          Submit
        </button>

        <button onClick={this.insertLine} id="clue-add-line-button">
          Add Line
        </button>
        <div>{clueConstruction}</div>
        <div>
          {this.props.timer ? (
            <Timer
